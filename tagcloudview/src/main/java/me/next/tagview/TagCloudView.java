...
public class TagCloudView extends ViewGroup {
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mBackground = a.getResourceId(R.styleable.TagCloudView_tcvBackground,
            DEFAULT_TEXT_BACKGROUND);
        mViewBorder = a.getDimensionPixelSize(R.styleable.TagCloudView_tcvBorder,
            DEFAULT_VIEW_BORDER);
        ...
        mRightImageResId = a.getResourceId(R.styleable.TagCloudView_tcvRightResId,
            DEFAULT_RIGHT_IMAGE);
        mSingleLine = a.getBoolean(R.styleable.TagCloudView_tcvSingleLine, DEFAULT_SINGLE_LINE);
        mShowRightImage = a.getBoolean(R.styleable.TagCloudView_tcvShowRightImg,
            DEFAULT_SHOW_RIGHT_IMAGE);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY ){
            ...
            setMeasuredDimension(
                sizeWidth,(sizeHeight>totalHeight)?totalHeight:sizeHeight );
        }else{
            setMeasuredDimension(
                sizeWidth, totalHeight);
        }
    ...
    private int getSingleTotalHeight(int totalWidth, int totalHeight) {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child.getTag() != null && (int) child.getTag() == TYPE_TEXT_NORMAL) {
                if (totalWidth + mTagBorderHor + mViewBorder + mViewBorder + endTextWidth +
     ...
     */
    public int getMultiTotalHeight(int totalWidth, int totalHeight) {
        int childWidth;
        int childHeight;
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
