...
import com.zhongmeban.attentionmodle.fragment.FragmentAttentionAddOperationStep2;
...
import com.zhongmeban.attentionmodle.fragment.FragmentAttentionAddOperationStep1;
import com.zhongmeban.attentionmodle.fragment.FragmentAttentionSurgeryMethodsByDisease;
 ...
 */
public class ActivityAttentionAddOperation extends BaseActivity {

    private FragmentAttentionAddOperationStep1 fragmentAttentionAddOperationStep1;
    private FragmentAttentionAddOperationStep2 fragmentAttentionAddOperationStep2;
    private FragmentAttentionSurgeryMethodsByDisease fragmentAttentionSurgeryMethodsByDisease;//其他设置Fragment
    private FragmentManager fragmentManager;
    private FragmentTransaction fragmentTransaction;
    private int step = 1;
    private int surgeryRecordId;//化疗ID
...
//    public List<AttentionOtherTherapeutic> surgeryMethodsList
//            = new ArrayList<AttentionOtherTherapeutic>();//其他设置 手术项
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ISFROMEHOSP = intent.getBooleanExtra("ISFROMEHOSP",false);
        surgeryRecordId = intent.getIntExtra("surgeryMethodId",0);
        inTime = intent.getIntExtra("inTime",0);
        outTime = intent.getIntExtra("outTime",0);

        initTitle();

        fragmentAttentionAddOperationStep1 = new FragmentAttentionAddOperationStep1();
//        fragmentAttentionAddOperationStep2 = new FragmentAttentionAddOperationStep2();
        fragmentAttentionSurgeryMethodsByDisease = new FragmentAttentionSurgeryMethodsByDisease();

        if (ISEDIT){//手术编辑进入
            getDBData(surgeryRecordId);
        }else if (ISFROMEHOSP &&surgeryRecordId>0){//住院编辑进入
            getDBData(surgeryRecordId);
        }else {//添加手术项
            ...
            if (mCreateSurgeryRecordBody != null){
                ...
                title.slideCentertext("编辑手术记录");

                surgeryRecordId = (int) createSurgeryRecordBody.getSurgeryRecord().getId();
...
//                createSurgeryRecordBody.setSurgeryRecord(surgeryRecordItem);
            ...
            fragmentTransaction.add(R.id.fl_content, fragmentAttentionAddOperationStep1);
            fragmentTransaction.commit();
        }


//        getSurgeryMethodsByDiseaseId();
    }
    ...
    public void notifyFragment(int step) {
        ...
        switch (step) {
            case 1:
                fragmentTransaction = fragmentManager.beginTransaction();
                fragmentTransaction.replace(R.id.fl_content, fragmentAttentionAddOperationStep1);
                fragmentTransaction.commit();
                break;
            case 2:
                fragmentTransaction = fragmentManager.beginTransaction();
//                fragmentTransaction.replace(R.id.fl_content, fragmentAttentionAddOperationStep2);
                fragmentTransaction.commit();
                break;
            case 3:
                fragmentTransaction = fragmentManager.beginTransaction();
