...
public class AttentionAddHospOperationActivity extends BaseActivityToolBar implements View.OnClickListener, AttentionAddHospOperationContract.View {

    private Activity mContext = AttentionAddHospOperationActivity.this;
    private AttentionAddHospOperationContract.Presenter presenter;
    ...
    protected void initView() {
        ...
        new AttentionAddHospOperationPresenter(mContext, this);
        presenter.start();
    }

    @Override
    public void setPresenter(AttentionAddHospOperationContract.Presenter presenter) {
        if (presenter != null) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.ev_hosp:
                //治疗医院
                Intent hospIntent = new Intent(mContext, AttentionHospChooseActivity.class);
                startActivityForResult(hospIntent, 1);
                break;
            case R.id.ev_time:
                ...
                if (!TextUtils.isEmpty(evDoct.getEditString())) {
                    if (!StringUtils.checkName(evDoct.getEditString())) {
                        ...
                        if (presenter.canCommit()) {
                            ...
                            setResult(AttentionAddHospOperationPresenter.AddHospOperationOK,
                                    presenter.backHospitalized(intent));
                            finish();
                        }
                    }
                } else {
